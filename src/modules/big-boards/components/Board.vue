...
const emits = defineEmits(['deleteBoard'])
...
<template>
  <div class="bg-base hover:bg-light duration-300 rounded-md h-64 w-full drop-shadow-lg drop-shadow-light cursor-pointer group transform -skew-x-12 overflow-hidden">
    ...
    <div class="w-full h-7/12 flex flex-col gap-2 items-center text-white transform skew-x-12 relative group p-4">
      <div class="absolute top-2 right-2 hidden group-hover:inline-block">
        <base-button class="bg-red-500 hover:bg-red-700 !px-2 !py-1 flex justify-center" @click="deleteBoard">
      ...
      <div class="flex flex-col justify-center items-center gap-1">
        ...
        <span class="text-sm">{{ dayjs(board.created_at).format('dddd, MMMM D, YYYY h:mm A	') }}</span>
      </div>
      ...
      <div v-if="board.updated_at" class="flex flex-col justify-center items-center gap-1">
        ...
        <span class="text-sm">{{ dayjs(board.updated_at).format('dddd, MMMM D, YYYY h:mm A	') }}</span>
      </div>
